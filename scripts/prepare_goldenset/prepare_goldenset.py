...
def generate_for_document(
    model, source_path: Path, relative_path: Path, question_count: int
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.questions_per_document < 1:
        print("error: --questions-per-document must be at least 1", file=sys.stderr)
        return 2
    ...
    for source_path in files:
        ...
        if output_path.exists() and not args.overwrite:
            print(f"skip: {relative_path} (output exists; use --overwrite)")
            try:
            ...
            continue
        print(f"generate: {relative_path}")
        try:
            result, warnings = generate_for_document(
                model, source_path, relative_path, args.questions_per_document
            )
